...
@interface FrogOpenUDID (Private)
...
//
+ (NSString*) value {
    return [FrogOpenUDID valueWithError:nil];
}

+ (NSString*) valueWithError:(NSError **)error {
    NSLog(@"kOpenUDIDSessionCache=%@", kOpenUDIDSessionCache);
...
//    }
    ...
    for (int n=0; n<kOpenUDIDRedundancySlots; n++) {
        ...
        if (slotPB==nil) {
            ...
            NSDictionary* dict = [FrogOpenUDID _getDictFromPasteboard:slotPB];
    ...
    if (openUDID==nil) {        
        if (mostReliableOpenUDID==nil) {
            ...
            //
            openUDID = [FrogOpenUDID _generateFreshOpenUDID];
        } else {
    ...
    if (availableSlotPBid!=nil && (myRedundancySlotPBid==nil || [availableSlotPBid isEqualToString:myRedundancySlotPBid])) {
        ...
        if (openUDID && localDict)
            [FrogOpenUDID _setDict:localDict forPasteboard:slotPB];
    }
